...
function seedIfEmpty() {
  ...
  const sample = [
    // Cameras
    { name: 'Dome Camera 2MP', categoryId: 'cat-cctv', price: 1499.00, image: 'images/domecamera.jpg' },
    { name: 'Bullet Camera 5MP', categoryId: 'cat-cctv', price: 2499.00, image: 'images/bullet-camera.jpg' },
...
function bindEvents() {
  ...
  const imageBrowse = byId('product-image-browse');
  if (imageBrowse) {
    imageBrowse.addEventListener('change', (e) => {
      const file = e.target.files?.[0];
      if (!file) return;
      byId('product-image').value = `images/${file.name}`;
    });
  }
  byId('btnResetForm').addEventListener('click', () => {
    byId('product-id').value = '';
    byId('product-form').reset();
    const browse = byId('product-image-browse');
    if (browse) browse.value = '';
  });
